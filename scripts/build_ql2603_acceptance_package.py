...
def _acceptance_plan(boards: tuple[str, ...], commit: str) -> str:
    table = "\n".join(
...
2. 确认解析结果恰好为 3 块板、15 个 session；时间点为 T0、T1、T2、倒数第二次、末次。
3. 开始计算，确认三块板分别完成；记录 CPU/GPU 类型、每板耗时及任何告警。
...
def build_package(
    ...
    *,
    ...
    (output_root / "ACCEPTANCE_PLAN.md").write_text(_acceptance_plan(boards, commit), encoding="utf-8")
    metadata = {

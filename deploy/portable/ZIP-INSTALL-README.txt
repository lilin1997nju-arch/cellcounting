...
- CPU版跳过NVIDIA和WMI显卡查询；PyTorch仅探测一次，并每10秒显示加载进度；
- 创建并保留 Workspace；
...
四、卸载
--------
- 可在 Windows“设置 → 应用”中选择 Cell Vision 卸载，也可双击安装目录根部的
  Uninstall-CellVision.cmd。
- 卸载会停止并删除当前目录对应的 CellVisionDesktopProduction 服务、桌面/开始菜单快捷方式
  和程序文件；如果同名服务属于另一个安装目录，卸载器会拒绝修改该服务。
- Workspace及其中全部历史项目默认保留在原目录，不提供自动删除数据选项。确认不再需要数据时，
  请在卸载完成并备份后手工处理 Workspace。

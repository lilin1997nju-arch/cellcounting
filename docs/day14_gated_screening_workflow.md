...
先用计算量较低的 Day14 成片生长检测排除没有生长起来的孔，只对阳性孔运行 T0～T2 的实例识别、分类与时序校正。末点前一帧只作为完整孔视野的人工生长证据，不再尝试定位代表区域，也不会默认放大。
...
3. Day14 无明显成片生长：`no_obvious_growth`，跳过T0～T2深度计算。
...
这些内容只作为说明和审核优先级，不应把单细胞来源强行改成多细胞来源，也不能用末点前一帧推翻末点阳性。
...
- `day7_regions_json`：兼容旧结果的保留字段；新流程固定输出空数组。

...
`CellVision-Setup-0.2.0-x64.exe` 是面向 Windows 10/11 x64 的单文件离线安装程序，包含：
...
如果在相同目录覆盖安装，已有 Workspace 会被自动保留并重新索引。如果旧 Workspace 位于其他目录，可在客户端“维护 → 导入历史 Workspace”中选择旧目录。导入时会：
...
也可以将旧 Workspace 复制到安装目录后，使用“维护 → 修复/重新扫描项目目录”。
...
构建脚本会覆盖最新源码和审核界面，下载并校验微软签名的 VC++ x64 运行库，然后生成 `electron\dist\CellVision-Setup-0.2.0-x64.exe`。
